...
def resource_path(relative_path):
    """Radi i za dev i za PyInstaller .exe"""
    try:
...
if usb_dependencies_available:
        ...
        try:
            if os.name == 'nt':
                result = subprocess.run(
                    f"fsutil fsinfo volumeinfo {drive_path}:",
                    shell=True,
                    capture_output=True,
                    text=True
                )
                return "FAT32" in result.stdout
    ...
    def format_fat32(drive_path):
        try:
            usb_queue.put(("status", tr("preparing_structure")))
            ...
            if os.name == 'nt':
                ...
                if is_fat32(f"{drive_letter}:"):
                    ...
                    try:
                        return "FAT32" in subprocess.check_output(
                ...
                subprocess.run(["sudo", "umount", drive_path], check=False)
                
                for i in range(2):
                    try:
                        subprocess.run(
                            ["sudo", "mkfs.fat", "-F", "32", "-n", "CAR_MUSIC", drive_path], 
                            check=True
                        )
                        time.sleep(2)
                        return True
    ...
    def create_car_friendly_structure(dest_path, source_folder):
        try:
            ...
            os.makedirs(os.path.join(dest_path, "MUSIC"), exist_ok=True)
            os.makedirs(os.path.join(dest_path, "PLAYLISTS"), exist_ok=True)
            
            autorun_path = os.path.join(dest_path, "AUTORUN.INF")
            with open(autorun_path, "w") as f:
                f.write("[autorun]\n")
                f.write("open=start_radio.bat\n")
                f.write("icon=radio.ico\n")
                f.write("label=CAR_MUSIC\n")
            
            bat_path = os.path.join(dest_path, "start_radio.bat")
            with open(bat_path, "w") as f:
                f.write("@echo off\n")
                f.write("echo Starting car radio interface...\n")
                f.write("start MUSIC\n")
            
            create_main_playlist(dest_path, source_folder)
            create_radio_icon(os.path.join(dest_path, "radio.ico"))
    ...
    def create_main_playlist(dest_path, source_folder):
        playlist_path = os.path.join(dest_path, "PLAYLISTS", "ALL_SONGS.m3u")
        music_files = []
    ...
    def create_radio_icon(icon_path):
        try:
            ...
            draw.ellipse([(15, 10), (50, 45)], fill='blue')
            draw.rectangle([(30, 45), (35, 60)], fill='blue')
            draw.rectangle([(40, 35), (45, 50)], fill='blue')
    ...
    def enhance_metadata(file_path):
        try:
            ...
            try:
                ...
                print(f"Metadata error for {file_path}: {str(e)}")
                return False
        except Exception as e:
    ...
    def copy_with_progress(src, dest):
        if not os.path.exists(src):
            usb_queue.put(("error", tr("source_folder_error")))
            return False
        ...
        for file_path in all_files:
            try:
                ...
                dest_path = os.path.join(dest, "MUSIC", rel_path)
                
                os.makedirs(os.path.dirname(dest_path), exist_ok=True)
                shutil.copy2(file_path, dest_path)
                
                if enhance_metadata(dest_path):
                    metadata_success += 1
                
                copied_files += 1
                progress_percent = int((copied_files / total_files) * 100)
                usb_queue.put(("progress", copied_files, total_files, progress_percent))
            except Exception as e:
    ...
    def burn_to_usb_thread(source_folder, usb_path):
        global current_usb_copied, current_usb_total
        try:
            ...
            usb_queue.put(("status", tr("burning")))
            usb_queue.put(("progress", 0, 0, 0))
            ...
            if eject_drive(usb_path):
                usb_queue.put(("status", tr("usb_ejected")))
            else:
                usb_queue.put(("status", tr("usb_eject_failed")))
            
            usb_queue.put(("success", tr("usb_ready")))
            
        except Exception as e:
            usb_queue.put(("error", f"{tr('error_burning')}: {str(e)}"))
        finally:
    ...
    def burn_to_usb():
        ...
        threading.Thread(
            target=burn_to_usb_thread, 
            args=(source_folder, usb_path),
            daemon=True
        ).start()
            ...
            for partition in psutil.disk_partitions():
                if 'removable' in partition.opts.lower():
                    try:
                        ...
                        label = f"{partition.mountpoint} ({free_gb:.1f}GB free of {total_gb:.1f}GB)"
                        drives.append((partition.mountpoint, label))
                    except:
                        drives.append((partition.mountpoint, partition.mountpoint))
        except:
            pass
    ...
    def burn_to_usb():
        messagebox.showerror(tr("error"), "USB burning requires additional dependencies")
...
try:
    ...
    logo_img = PhotoImage(file=resource_path('logo.gif'))
